...
df$outcome.1 <- 1 - 1.2 * df$LV.EDV + 1.0 * df$LDL + 0.8 * df$cholesterol + 0.6 * df$free.cholesterol + 0.9 * df$triglycerides + rnorm(n)
...
```{r formatting_data-2}
# formatting data
format_results <- format_med(med_results, estimate = 'average')
```
