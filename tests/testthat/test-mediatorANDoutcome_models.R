...
test_that(
  ...
  code = {
    m <- data_models(outcome = 'MVO', mediator = c('LV.EDV', 'RV.EDV'), treatment = 'LDL')
...
test_that(
  ...
  code = {
    m <- data_models(outcome = c('MVO','Chol'), mediator = c('LV.EDV', 'RV.EDV'), treatment = c('LDL', 'FC'))
...
test_that(
  ...
  code = {
    m <- data_models(outcome = 'MVO', mediator = 'LV.EDV', treatment = 'LDL',
                     col_m = 'form.m', col_y = 'form.y')
...
test_that(
  ...
  code = {
    expect_error(data_models(outcome = data.frame(x='MVO'), mediator = 'LV.EDV', treatment = 'LDL'),
                 "Please, provide a vector for the outcome")
    expect_error(data_models(outcome = 'MVO', mediator = data.frame(x='LV.EDV'), treatment = 'LDL'),
                 "Please, provide a vector for the mediator")
    expect_error(data_models(outcome = 'MVO', mediator = 'LV.EDV', treatment = data.frame(x='LDL')),
                 "Please, provide a vector for the treatment")
    expect_error(data_models(outcome = 'MVO', mediator = 'LV.EDV', treatment = 'LDL',
                             col_m = 1, col_y = 'form.y'),
                 "col_m and col_y must be character strings")
    expect_error(data_models(outcome = 'MVO', mediator = 'LV.EDV', treatment = 'LDL',
                             col_m = 'same', col_y = 'same'),
                 "col_m and col_y must be different")
    expect_error(data_models(outcome = 'MVO', mediator = 'LV.EDV', treatment = 'LDL',
                             col_m = 'outcome', col_y = 'form.y'),
                 "col_m and col_y must not use reserved column names")
  }
)
