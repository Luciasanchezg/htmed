...
test_that(
  ...
  code = {
    ...
    visual_outcome1_noadj <- scatter_htmed(mediation.form = format_1out, outcome = 'outcome.1')
...
test_that(
  ...
  code = {
    expect_error(
      scatter_htmed(mediation.form = format_1out$outcome.1, outcome = 'outcome.1'),
    ...
    expect_error(
      scatter_htmed(mediation.form = format_1out, outcome = 'outcome.1', mediator = lm),
      regexp = "mediator is not a character"
    )
    expect_error(
      scatter_htmed(mediation.form = format_1out, outcome = 'outcome.1', treatment = lm),
      regexp = "treatment is not a character"
    )
    expect_error(
      scatter_htmed(mediation.form = format_1out, outcome = 'outcome.1', pval = '0.05', pval.column = 'p-value_Prop._Mediated_(average)'),
      regexp = "pval is not a number"
    )
    expect_error(
      scatter_htmed(mediation.form = format_1out, outcome = 'outcome.1', pval = '0.05', pval.column = lm),
      regexp = "pval.column is not a character"
    )
    expect_error(
      scatter_htmed(mediation.form = format_1out, outcome = 'outcome.1', prop.med = lm),
      regexp = "prop.med is not a character"
    )
    expect_error(
      scatter_htmed(mediation.form = format_1out, outcome = 'outcome.1', acme = lm),
      regexp = "acme is not a character"
    )
    expect_error(
      scatter_htmed(mediation.form = format_1out, outcome = 'outcome.1', data.split = lm),
...
test_that(
  ...
  code = {
    ...
    expect_error(
      scatter_htmed(mediation.form = format_1out, outcome = 'outcome.1', treatment = 'outcome.2'),
      regexp = "treatment and/or mediator columns do not exist"
    )
    expect_error(
      scatter_htmed(mediation.form = format_1out, outcome = 'outcome.1', mediator = 'outcome.2'),
      regexp = "treatment and/or mediator columns do not exist"
    )
    expect_error(
      scatter_htmed(mediation.form = format_1out, outcome = 'outcome.1', pval.column = 'outcome.1'),
      regexp = "pval.column is not in the dataset"
    )
    expect_error(
      scatter_htmed(mediation.form = format_1out, outcome = 'outcome.1', pval = -3, pval.column = 'adj.p-value.by_outcome'),
      regexp = "Negative values not supported by pval"
    )
    expect_error(
      scatter_htmed(mediation.form = format_1out, outcome = 'outcome.1', prop.med = 'outcome.1'),
      regexp = "Wrong columns for proportion of mediation and/or ACME in the outcome chosen"
    )
    expect_error(
      scatter_htmed(mediation.form = format_1out, outcome = 'outcome.1', acme = 'outcome.1'),
      regexp = "Wrong columns for proportion of mediation and/or ACME in the outcome chosen"
    )
    expect_error(
      scatter_htmed(mediation.form = format_1out, outcome = 'outcome.1', data.split = 'outcome.1'),
...
test_that(
  ...
  code = {
    ...
    expect_error(
      scatter_htmed(mediation.form = format_1out, outcome = 'outcome.1', pval.column = 'adj.p-value.by_outcome', pval = 0),
